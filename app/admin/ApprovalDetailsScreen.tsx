...
const ApprovalDetailsScreen = ({ route, navigation }) => {
            ...
            if (type === 'event') {
                ...
                if (eventData) {
                    ...
                    if (eventData.needsVolunteers) {
                        ...
                        if (!oppSnap.empty) {
                            setOpportunity({ id: oppSnap.docs[0].id, ...oppSnap.docs[0].data() });
                        }
                    }
                }
            }
    ...
    const handleApprove = async () => {
        if (event) {
            await adminEventService.updateEvent(event.id, { approvalStatus: 'approved' });
        }
        if (opportunity) {
            await FirebaseOpportunityService.updateOpportunity(opportunity.id, { approvalStatus: 'approved' });
        }
    ...
    const handleReject = async () => {
        if (event) {
            await adminEventService.updateEvent(event.id, { approvalStatus: 'rejected' });
        }
        if (opportunity) {
            await FirebaseOpportunityService.updateOpportunity(opportunity.id, { approvalStatus: 'rejected' });
        }
    ...
    if (loading) {
        return <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#FDF6EC' }}><Text>Loading...</Text></View>;
    }
    if (!event) {
        return <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#FDF6EC' }}><Text>Event not found.</Text></View>;
    }

    return (
        <ScrollView style={styles.container}>
            <AdminHeroBox title='Approval Details' showBackButton customBackRoute='ApprovalManagement' />
            <View style={styles.card}>
                ...
                <Text style={styles.value}>{event.date ? (event.date.seconds ? new Date(event.date.seconds * 1000).toLocaleDateString() : new Date(event.date).toLocaleDateString()) : ''}</Text>
