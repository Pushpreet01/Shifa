// Reminder: Make sure to install expo-mail-composer and its types if not already installed.
...
// Add string index signature for TypeScript
const placeholderEmails: Record<string, string[]> = {
...
const AdminEmailScreen = () => {
  ...
  const fetchUserRole = (email: string) => {
    for (const role of roles) {
      if (placeholderEmails[role] && placeholderEmails[role].includes(email)) {
        return role;
      }
    }
    return "";
    ...
    <SafeAreaView style={styles.container}>
      ...
      <KeyboardAwareWrapper>
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <View style={styles.dropdownContainer}>
            ...
            {showRoleDropdown && (
              <View style={styles.dropdownList}>
                {["Particular User", ...roles].map((role: string) => (
                  <TouchableOpacity
                    key={role}
                    onPress={() => {
          ...
          {/* Show emails for selected role (not Particular User) */}
          {selectedRole && selectedRole !== "Particular User" && (
            <View style={styles.emailListContainer}>
              ...
              {placeholderEmails[selectedRole]?.length > 0 ? (
                placeholderEmails[selectedRole].map(
          ...
          {selectedRole === "Particular User" && (
            <>
              ...
              <Text style={styles.label}>
                User Role: {specificUserRole || "Not found"}
              </Text>
            </>
          )}
